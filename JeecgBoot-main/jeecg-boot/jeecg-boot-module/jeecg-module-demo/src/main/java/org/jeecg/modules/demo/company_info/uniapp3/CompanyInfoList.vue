...
layout: 'default',
style: {
navigationBarTitleText: '企业信息管理',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="企业信息管理"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CompanyInfoData';
defineOptions({
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/company_info/companyInfo/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/company_info/companyInfo/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
