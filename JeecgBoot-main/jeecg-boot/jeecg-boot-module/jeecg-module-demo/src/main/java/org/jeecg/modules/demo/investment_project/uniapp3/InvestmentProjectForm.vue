      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['projectCode']"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('产业类型')"
                 labelWidth="100px"
                 type="list"
                 name='industryType'
                 dict="sys_dict,	industry,	industry"
                v-model="myFormData['industryType']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('所在地块')"
                 labelWidth="100px"
                 type="list"
                 name='landParcelId'
                 dict="	land_parcel,parcel_name,	id"
                v-model="myFormData['landParcelId']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('地块状态')"
                 labelWidth="100px"
                 type="list"
                 name='landStatus'
                 dict="sys_dict,land_status,land_status"
                v-model="myFormData['landStatus']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['investmentAmount']"
                   :label="get4Label('投资额（万元）')"
                   name='investmentAmount'
                   prop='investmentAmount'
                   placeholder="请选择投资额（万元）"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['contactPhone']"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('项目简介')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['projectIntro']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('项目意向')"
                 labelWidth="100px"
                 type="textarea"
                 name='projectIntention'
                 prop='projectIntention'
                 clearable
                 :maxlength="300"
                v-model="myFormData['projectIntention']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('预计投产时间')"
                  labelWidth="100px"
                  type="date"
                  name='expectedStartDate'
                  v-model:value="myFormData['expectedStartDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('实际投产时间')"
                  labelWidth="100px"
                  type="date"
                  name='actualStartDate'
                  v-model:value="myFormData['actualStartDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('项目状态')"
                 labelWidth="100px"
                 type="list"
                 name='projectStatus'
                 dict="sys_dict,project_status,project_status"
                v-model="myFormData['projectStatus']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['investmentUnit']"
                  :label="get4Label('引资单位')"
                  name='investmentUnit'
                  prop='investmentUnit'
                  placeholder="请选择引资单位"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('InvestmentProjectList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/investment_project/investmentProject/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/investment_project/investmentProject/edit':'/investment_project/investmentProject/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
