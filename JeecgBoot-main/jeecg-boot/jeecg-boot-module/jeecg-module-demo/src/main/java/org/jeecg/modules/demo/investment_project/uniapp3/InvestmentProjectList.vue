...
<template>
  <PageLayout navTitle="招商项目管理"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './InvestmentProjectData';
defineOptions({
  name: 'InvestmentProjectList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/investment_project/investmentProject/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/investment_project/investmentProject/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
