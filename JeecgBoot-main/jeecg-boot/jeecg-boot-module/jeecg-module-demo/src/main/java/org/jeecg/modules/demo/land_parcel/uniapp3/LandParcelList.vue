...
import {columns} from './LandParcelData';
defineOptions({
  name: 'LandParcelList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/land_parcel/landParcel/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/land_parcel/landParcel/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
